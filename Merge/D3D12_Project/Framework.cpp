...
void Framework::OnInit(HINSTANCE hInstance, UINT width, UINT height)
{
    ...
    // 네트워크 콜백 설정
    m_networkManager.SetLoginSuccessCallback([this](int clientID, const std::string& username) {
        OnLoginSuccess(clientID, username);
    });
    m_networkManager.SetLoginFailedCallback([this](const std::string& errorMessage) {
        OnLoginFailed(errorMessage);
    });
    
    // 로그인 화면 상태로 설정
    m_loginUI.SetState(UIState::LOGIN_SCREEN);
}
...
{
    if (m_isInLoginScreen) {
        // 로그인 화면에서는 D3D12 렌더링을 완전히 건너뛰고 Windows 컨트롤만 표시
        // 아무것도 하지 않음 (깜빡임 방지)
        return;
    }
    ...
    try {
        ...
        // 게임 화면에서 마우스 커서 숨기기 유지
        ShowCursor(FALSE);
        
        // 게임 화면에서는 Windows 컨트롤 완전 숨김
        HWND hwnd = m_win32App->GetHwnd();
        ...
        OutputDebugString(L"[Framework] Exception in OnRender\n");
    }
    catch (...) {
        OutputDebugString(L"[Framework] Unknown exception in OnRender\n");
    }
}
...
void Framework::BuildDsvDescriptorHeap()
{
    ...
    dsvHeapDesc.NumDescriptors = 1 + 1; // 1 �� shdowmap��
...
void Framework::EnableNetwork(const char* serverIP, int port)
{
    if (m_isNetworkEnabled) {
        DisableNetwork();
    }
    
    Scene* currentScene = &GetScene(GetCurrentSceneName());
    if (m_networkManager.Initialize(serverIP, port, currentScene)) {
        m_isNetworkEnabled = true;
    }
}

void Framework::DisableNetwork()
{
    if (m_isNetworkEnabled) {
...
void Framework::OnLoginSuccess(int clientID, const std::string& username) {
    ...
    // 로그인 UI 완전 제거
    m_loginUI.DestroyControls();
    
    // 게임 화면에서는 마우스 커서 완전 숨기기
    ShowCursor(FALSE);
    ...
    OutputDebugString(L"[Framework] Login UI completely removed\n");
}
...
void Framework::OnConnectToServer(const std::wstring& serverIP, int port) {
    ...
    if (m_scenes.find(L"BaseScene") != m_scenes.end() && m_networkManager.Initialize(ipStr.c_str(), port, m_scenes[L"BaseScene"])) {
        // 네트워크 활성화
        m_isNetworkEnabled = true;
        ...
        std::thread([this]() {
            ...
            if (m_networkManager.IsRunning()) {
                ...
                if (!usernameStr.empty()) {
                    m_networkManager.SendLoginRequest(usernameStr);
                } else {
