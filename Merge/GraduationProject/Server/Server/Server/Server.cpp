    ...
    switch (header->type) {
        ...
        case PACKET_STAGE_CHANGE: {
            ...
            if (strcmp(pkt->stageName, "God") == 0) {
                // 클라이언트가 이미 랜덤 퍼즐 패턴을 생성했으므로 서버에서 덮어쓰지 않음
                // SendPuzzleStatusToClient(clientID);
                std::cout << "[StageChange] Client " << clientID << " entered God stage - puzzle pattern already generated by client" << std::endl;
            }
